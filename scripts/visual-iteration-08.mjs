...
const visualIteration = process.env.VISUAL_ITERATION ?? "iteration-08";
const visualStage = process.env.VISUAL_STAGE ?? "after";
const appUrl = "http://127.0.0.1:3088";
const storybookUrl = "http://127.0.0.1:6088";
const root = path.resolve("screenshots", visualIteration, visualStage);
...
async function historyBack(page, selector) { await page.evaluate(() => window.history.back()); if (selector) await page.locator(selector).waitFor({ state: "detached", timeout: 5000 }); await settle(page, 320); }
...
await selectIndex(page, 2); await screenshot(page, "02b-day-08.png");
await selectIndex(page, 3); await screenshot(page, "02c-day-09.png");
...
if (visualIteration === "iteration-09") { await page.evaluate(() => document.documentElement.style.setProperty("--ds-safe-bottom", "24px")); await page.locator(".ds-bottom-navigation").screenshot({ path: path.join(output, "07b-android-safe-bottom.png") }); await page.evaluate(() => document.documentElement.style.removeProperty("--ds-safe-bottom")); }
...
await page.getByRole("button", { name: /Ver detalles: Sky Garden/i }).click(); await page.locator(".detail-layer").waitFor(); await settle(page, 700); await screenshot(page, "12-sky-garden-detail.png");
...
if (visualIteration === "iteration-09") { await hardRock.getByRole("button", { name: "Detalle" }).click(); await page.locator(".ds-sheet").waitFor(); await settle(page, 600); await screenshot(page, "15b-place-detail-sheet.png"); await historyBack(page, ".ds-sheet"); await hardRock.getByRole("button", { name: /Añadir a un día|Asignado al/i }).click(); await settle(page, 500); await screenshot(page, "15c-assignment-step-1.png"); await page.locator(".assignment-days > button").nth(1).click(); await settle(page, 420); await screenshot(page, "15d-assignment-step-2.png"); await historyBack(page); await historyBack(page, ".ds-sheet"); }
...
const stories = visualIteration === "iteration-09" ? [["core-bottomnavigation-iteration-09--days-active", "01-navigation-days.png"], ["core-bottomnavigation-iteration-09--saved-active", "02-navigation-saved.png"], ["core-bottomnavigation-iteration-09--trip-active", "03-navigation-trip.png"], ["core-bottomnavigation-iteration-09--focus", "04-navigation-focus.png"], ["patterns-daydeck-iteration-09-motion--no-dark-flash", "05-deck-no-dark-flash.png"], ["patterns-backnavigation-iteration-09--activity-detail", "06-back-activity.png"], ["patterns-backnavigation-iteration-09--modal-flow", "07-back-modal.png"], ["patterns-backnavigation-iteration-09--direct-entry-fallback", "08-back-fallback.png"]] : [["expression-daycover-atmospheric--flat-vs-atmospheric", "01-atmospheric.png"], ["core-bottomnavigation-iteration-09--days-active", "02-navigation.png"], ["patterns-product-patterns--journey", "03-day-patterns.png"], ["patterns-product-patterns--saved", "04-saved-place-card.png"], ["patterns-product-patterns--trip", "05-flight-card.png"], ["experiments-day-navigation-vertical-stack--comparison-only", "06-vertical-stack.png"]];
...
if (engineName === "chromium") {
  ...
  if (visualIteration === "iteration-09") {
    await selectIndex(videoPage, 1); await videoPage.waitForTimeout(450);
    ...
    await selectIndex(videoPage, 1); await drag(videoPage, 2, -118, 9, true); await videoPage.locator(".day-open-layer").waitFor(); await videoPage.waitForTimeout(550);
    const videoSky = videoPage.getByRole("button", { name: /Ver detalles: Sky Garden/i }); await videoSky.scrollIntoViewIfNeeded(); await videoSky.click(); await videoPage.locator(".detail-layer").waitFor(); await videoPage.waitForTimeout(650);
    await historyBack(videoPage, ".detail-layer"); await videoPage.waitForTimeout(450);
    await historyBack(videoPage, ".day-open-layer"); await videoPage.waitForTimeout(450);
    await videoPage.getByRole("button", { name: "Guardados" }).click(); await videoPage.waitForTimeout(550);
    const videoHardRock = videoPage.locator(".ds-saved-place-card").filter({ hasText: "Hard Rock Cafe" }).first(); await videoHardRock.scrollIntoViewIfNeeded(); await videoPage.waitForTimeout(350); await videoHardRock.getByRole("button", { name: "Detalle" }).click(); await videoPage.waitForTimeout(600);
    await historyBack(videoPage, ".ds-sheet"); await videoPage.waitForTimeout(400);
    await videoPage.getByRole("button", { name: "Viaje" }).click(); await videoPage.waitForTimeout(500); await videoPage.getByRole("button", { name: "Días" }).click(); await videoPage.waitForTimeout(500); await videoPage.getByRole("button", { name: "Guardados" }).click(); await videoPage.waitForTimeout(600);
  } else {
  ...
  if (videoPath) { const target = path.join(videoOutput, `${visualIteration}-mobile-flow.webm`); await fs.copyFile(videoPath, target); videoPath = target; }
}
