...
export function Sheet({ title, kicker, closeLabel, children, footer, onClose, wide = false, handle = true }: SheetProps) {
  ...
  return <motion.div ref={backdropRef} className="ds-sheet-backdrop" role="presentation" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><motion.section ref={sheetRef} style={{ x: backSwipe.x }} className={`ds-sheet${wide ? " ds-sheet--wide" : ""}`} role="dialog" aria-modal="true" aria-labelledby={titleId} initial={reducedMotion ? { opacity: 0 } : { y: "100%" }} animate={reducedMotion ? { opacity: 1 } : { y: 0 }} exit={reducedMotion ? { opacity: 0 } : { y: "100%" }} transition={reducedMotion ? { duration: 0 } : modalMotion.spring}>
    <div className="app-back-swipe-zone" aria-hidden="true" {...backSwipe.zoneProps} />{handle ? <DragHandle label="Indicador de panel" tabIndex={-1} /> : null}<header className="ds-sheet__header"><div>{kicker ? <p>{kicker}</p> : null}<h2 id={titleId}>{title}</h2></div><CloseControl autoFocus label={closeLabel} onClick={onClose} /></header><div className="ds-sheet__scroll" data-navigation-scroll={`sheet:${title}`} onFocusCapture={revealField}>{children}</div>{footer ? <footer className="ds-sheet__footer">{footer}</footer> : null}
