...
export function SavedPlaceCard({ title, category, area, media, tags = [], assignedLabel, onAssign, mapsHref, onDetail, onEdit }: { title: string; category: string; area?: string; media?: ReactNode; tags?: readonly string[]; assignedLabel?: string; onAssign?: () => void; mapsHref?: string; onDetail?: () => void; onEdit?: () => void }) {
  const visibleTags = tags.slice(0, 2);
  return <article className="ds-saved-place-card">
    <div className="ds-saved-place-card__media">{media ?? <div className="ds-saved-place-card__fallback" aria-hidden="true" />}<span className="ds-saved-place-card__saved" aria-label="Guardado"><span aria-hidden="true">♥</span></span></div>
    ...
    <div className="ds-saved-place-card__actions"><button type="button" className="ds-saved-place-card__primary" onClick={onAssign}><span aria-hidden="true">+</span><span>{assignedLabel ?? "Añadir a un día"}</span><span aria-hidden="true">→</span></button><div className="ds-saved-place-card__secondary">{mapsHref ? <a href={mapsHref} target="_blank" rel="noreferrer">Google Maps</a> : <span />}{onDetail ? <button type="button" onClick={onDetail}>Detalle</button> : null}<details><summary aria-label={`Más acciones para ${title}`}>···</summary>{onEdit ? <button type="button" onClick={onEdit}>Editar</button> : null}</details></div></div>
  </article>;
}
...
export function FlightTicketCard({ origin, destination, startTime, endTime, meta, icon }: { origin: string; destination: string; startTime?: string; endTime?: string; meta: string; icon?: ReactNode }) { return <article className="ds-flight-ticket"><div><span>{origin}</span><strong>{startTime}</strong></div><span className="ds-flight-ticket__route" aria-hidden="true">{icon ?? "→"}</span><div><span>{destination}</span><strong>{endTime}</strong></div><p>{meta}</p></article>; }
