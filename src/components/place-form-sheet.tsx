...
export function PlaceFormSheet({ place, onSave, onDelete, onClose }: PlaceFormSheetProps) {
  ...
  return (
    <MobileSheet title={place ? es.forms.editPlace : es.forms.addPlace} kicker="Guardados / Editor local" closeLabel={es.forms.close} onClose={onClose}>
      <form className="editor-form" onSubmit={submit}>
        <label><span>{es.forms.name} *</span><input value={name} onChange={(event) => setName(event.target.value)} autoComplete="off" /></label>
        ...
        <label><span>{es.forms.maps}</span><input value={mapsQuery} onChange={(event) => setMapsQuery(event.target.value)} placeholder="Nombre o enlace" /></label>
        ...
        <div className="form-actions">
          <button className="primary-action" type="submit">{es.forms.save}</button>
          <button type="button" onClick={onClose}>{es.forms.cancel}</button>
        </div>
