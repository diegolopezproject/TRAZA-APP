...
function levelFor(status: ActivityStatus): ActivityLevel {
  if (status === "confirmed") return "anchor";
  if (status === "planned" || status === "flexible") return "intention";
...
export function PlanFormSheet({ day, days, places, plan, onChoosePlace, onSave, onDelete, onClose }: PlanFormSheetProps) {
  ...
  return (
    <MobileSheet title={plan ? es.forms.editPlan : es.forms.addPlan} kicker={`Días / ${day.date.slice(-2)} AGO`} closeLabel={es.forms.close} onClose={onClose} wide>
      {mode === "menu" ? (
      ...
      {mode === "saved" ? (
        <div className="saved-picker-list">
          <label className="picker-section-select"><span>{es.forms.section}</span><select value={section} onChange={(event) => setSection(event.target.value as DaySection)}>{Object.entries(es.forms.sections).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
          {places.map((place) => <button type="button" key={place.id} onClick={() => onChoosePlace(place.id, section)}>{place.media ? <MediaFrame media={place.media} sizes="64px" /> : <span className="mini-fallback">{place.name.slice(0, 2).toUpperCase()}</span>}<span><strong>{place.name}</strong><small>{place.area ?? "Londres"} · {es.saved.categories[place.category]}</small></span><PlusIcon /></button>)}
        </div>
      ) : null}

      {mode === "create" ? (
        <form className="editor-form" onSubmit={submit}>
          <label><span>{es.forms.name} *</span><input value={name} onChange={(event) => setName(event.target.value)} /></label>
          <div className="form-grid">
            <label><span>{es.forms.date}</span><select value={dayId} onChange={(event) => setDayId(event.target.value)}>{days.map((item) => <option key={item.id} value={item.id}>{item.date.slice(-2)} AGO · {weekdayEs(item)}</option>)}</select></label>
          ...
          <div className="form-grid">
            <label><span>{es.forms.time}</span><input type="time" value={time} onChange={(event) => setTime(event.target.value)} /></label>
            <label><span>{es.forms.category}</span><select value={category} onChange={(event) => setCategory(event.target.value)}><option value="attraction">Atracción</option><option value="tour">Tour</option><option value="museum">Museo</option><option value="entertainment">Espectáculo</option><option value="meal">Comida</option><option value="neighbourhood">Barrio</option></select></label>
          </div>
          ...
          <label><span>{es.forms.maps}</span><input value={mapsQuery} onChange={(event) => setMapsQuery(event.target.value)} /></label>
          {error ? <p className="form-error" role="alert">{error}</p> : null}
          <div className="form-actions"><button className="primary-action" type="submit">{es.forms.save}</button><button type="button" onClick={onClose}>{es.forms.cancel}</button></div>
