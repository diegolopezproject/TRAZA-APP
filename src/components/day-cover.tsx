...
export function DayCover({ day, index, active, onOpen }: DayCoverProps) {
  ...
  const rotateX = useTransform(y, [-130, 0], [reducedMotion ? 0 : 11, 0]);
  const scale = useTransform(y, [-130, 0], [reducedMotion ? 1 : .96, 1]);
  const dateY = useTransform(y, [-130, 0], [reducedMotion ? 0 : -18, 0]);
  const mediaY = useTransform(y, [-130, 0], [reducedMotion ? 0 : -9, 0]);
  ...
  return (
    <motion.article
      ...
      whileTap={active && !reducedMotion ? { scale: .992 } : undefined}
    >
      <header className="cover-kicker">
        <span>{editorial.meta}</span>
        <span>{String(index + 1).padStart(2, "0")} / {String(8).padStart(2, "0")}</span>
      </header>

      <motion.div className="cover-number" style={active ? { y: dateY } : undefined} aria-hidden="true">{number}</motion.div>
      <motion.div className="cover-motif-wrap" style={active ? { y: mediaY } : undefined}><DayMotif day={day} /></motion.div>
